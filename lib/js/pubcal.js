...
function PubCalController($scope, angularFire) {
  ...
  $scope.updateFilter = function() {
    console.log("called with " + $scope.searchstring);
    $scope.hide = [];
    if($scope.searchstring.length > 0) {
      for(e in $scope.events) {
	var cur = $scope.events[e];
	console.log(cur.name + " " + cur.short_name + " " + cur.description + 
	  " " + cur.categories);
	if((cur.name + " " + cur.short_name + " " + cur.description + 
	  " " + cur.categories).toLowerCase().indexOf($scope.searchstring.toLowerCase()) == -1) {
	  $scope.hide.push(cur);
	}
      }
    }
  ...
  $scope.updateView = function() {
    ...
    datelist = _.union(datelist, _.map(events, function(v) { if(_.contains($scope.hide, v)) return 0; else return v.start.month * 100 + v.start.day; }));
    datelist = _.union(datelist, _.map(events, function(v) { if(_.contains($scope.hide, v)) return 0; else return ((v.end.month < v.start.month ? 12 : 0) + v.end.month) * 100 + v.end.day; }));
    datelist.sort(function(a,b) { return a-b; });

    for(e in events) {
      if(_.contains($scope.hide, events[e]))
	continue;
